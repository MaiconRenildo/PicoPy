...
def _start_primitive_render_context(rect: tuple) -> tuple:
    """
...
def _finish_primitive_render_context(clip, target, pos, aux):
    """
...
def _execute_primitive_render(rect: tuple, draw_function, init_function=None, finish_function=None):
    ...
    if not REN:
        return
    init_function = init_function or _start_primitive_render_context
    finish_function = finish_function or _finish_primitive_render_context
...
def _execute_point_primitive_render(
    points: list[tuple],
    draw_function,
    count=None,
    start_function=None,
    ...
    Args:
        ...
        start_function: (Opcional) Função para iniciar o contexto de desenho.
                        Usa _start_point_primitive_render_context por padrão.
        finish_function: (Opcional) Função para finalizar o contexto de desenho.
                         Usa _finish_point_primitive_render_context por padrão.
    """
    if not REN or not points or (count is not None and count == 0):
        return
    start_function = start_function or _start_point_primitive_render_context
    finish_function = finish_function or _finish_point_primitive_render_context
    clip, target, primitive_dimensions, aux = start_function(points)
    draw_function(primitive_dimensions, count)
    finish_function(clip, target, primitive_dimensions, aux)
...
def pico_output_draw_line(p1, p2):
    ...
    """
    def _draw_function(primitive_dimensions, count):
    ...
    _execute_point_primitive_render(
        points=[p1, p2], 
...
def pico_output_draw_poly(vertices, count):
    ...
    """
    def _draw_function(primitive_dimensions, actual_count):
        import ctypes
