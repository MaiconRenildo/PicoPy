...
FPS = 60
FRAME_DELAY = 16 / 1000  # 1000ms / 60fps = ~16.6ms
...
def main():
    ...
    while running and not game_over: # Loop continua enquanto não for Game Over
        ...
        if not game_over:
            ...
            # Necessitaria de funções de texto para exibir mensagem, por enquanto só para de desenhar o jogo
            print("--- GAME OVER! --- (Nenhum texto exibido no jogo sem pico_output_text)")

        pico_app.pico_output_present() # Mostra o frame
        time.sleep(FRAME_DELAY) # Controla o FPS
