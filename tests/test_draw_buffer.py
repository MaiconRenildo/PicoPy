...
def test_draw_buffer_simple_square():
    ...
    try:
        ...
        pos = (10, 10)
        Pico.pico_output_draw_buffer(pos, buffer, dim)
        Pico.screenshot_and_compare("test_draw_buffer_simple_square.png")
    finally:
        Pico.pico_init(0)
...
def test_draw_buffer_with_transparent_pixel():
    ...
    try:
        ...
        buffer[buffer_index] = transparent_pixel # type: ignore
        pos = (0, 0) # Posição do canto superior esquerdo na tela
        Pico.pico_output_draw_buffer(pos, buffer, dim)
        Pico.screenshot_and_compare("test_draw_buffer_with_transparent_pixel.png")
    finally:
        Pico.pico_init(0)
...
def test_draw_buffer_with_offset_and_scale2():
    ...
    try:
        ...
        Pico.pico_set_zoom((100, 100)) # Garante zoom padrão
        Pico.pico_output_clear() # Limpa a textura antes de desenhar
        # Posição centralizada para zoom 100%: (64//2 - 5, 36//2 - 5) = (27, 13)
        pos_centro = (27, 13)
        Pico.pico_output_draw_buffer(pos_centro, buffer, dim)
        Pico.screenshot_and_compare("test_draw_buffer_fixed_pos_100_percent_zoom.png")

        # --- Teste 2: Zoom de 200%(Objeto deve ficar menor) ---
        Pico.pico_set_zoom((200, 200))
        Pico.pico_output_clear() # Limpa a textura antes de desenhar
        Pico.pico_output_draw_buffer(pos_centro, buffer, dim)
        Pico.screenshot_and_compare("test_draw_buffer_fixed_pos_200_percent_zoom.png")

        # --- Teste 3: Zoom de 50%(Objetos deve ficar maior) ---
        Pico.pico_set_zoom((50, 50))
        Pico.pico_output_clear() # Limpa a textura antes de desenhar
        Pico.pico_output_draw_buffer(pos_centro, buffer, dim)
        Pico.screenshot_and_compare("test_draw_buffer_fixed_pos_50_percent_zoom.png")
    finally:
